...
(async function () {
  ...
  const sello = v => !v ? '' : `<span class="ver ${v.estado}">${v.estado === 'verificada' ? 'verificada ' + v.fecha : v.estado === 'parcial' ? 'verificación parcial' : v.estado === 'espejo' ? 'vía espejo' : 'simulación propia'}</span>`;
  ...
  function select(n) {
    ...
    const item = (l, dir) => { const o = dir === 'out' ? l.target : l.source; return `<div class="edge-item"><span class="muted">${dir === 'out' ? '→' : '←'} ${esc(TIPO_V(l.tipo))}</span> <button data-id="${esc(o.id)}">${esc(nombre(o))}</button><div class="quote ${linkStyle(l) === 'aus' ? 'aus' : ''}">${esc(l.cita)}</div><div class="body-s muted" style="margin-top:4px">${l.fecha ? esc(l.fecha) + ' · ' : ''}${l.url ? `<a href="${esc(l.url)}" target="_blank" rel="noopener">${esc(l.fuente)}</a>` : esc(l.fuente)}${l.evidencia_visual ? ` · <a href="${esc(l.evidencia_visual)}" target="_blank" rel="noopener">logotipo</a>` : ''} <span title="${verTitulo(l.verificacion)}">${sello(l.verificacion)}</span></div></div>`; };
  ...
  window.beholder = {
    ver(id) {
      ...
      select(n); window.doppelNav?.irA('grafo', false);
      const card = document.querySelector('#grafo .graph-card'); setTimeout(() => { card.classList.remove('pulso'); void card.offsetWidth; card.classList.add('pulso'); }, 500);
    }
  };
  ...
  /* tabla de vínculos */
  $('#tabla-v').innerHTML = `<table class="data"><thead><tr><th>Origen</th><th>Vínculo</th><th>Destino</th><th>Cita verificada</th><th>Fecha</th><th>Fuente</th><th>Verificación</th></tr></thead><tbody>${links.map(l => `<tr><td>${esc(nombre(l.source))}</td><td>${esc(TIPO_V(l.tipo))}</td><td>${esc(nombre(l.target))}</td><td class="body-s">${esc(l.cita)}</td><td class="body-s">${esc(l.fecha)}</td><td class="body-s">${l.url ? `<a href="${esc(l.url)}" target="_blank" rel="noopener">${esc(l.fuente)}</a>` : esc(l.fuente)}${l.evidencia_visual ? `<br><a class="body-s" href="${esc(l.evidencia_visual)}" target="_blank" rel="noopener">logotipo</a>` : ''}</td><td class="body-s" title="${verTitulo(l.verificacion)}">${sello(l.verificacion)}</td></tr>`).join('')}</tbody></table>`;
  ...
  $('#fuentes').innerHTML = `<table class="data"><thead><tr><th>Fuente</th><th>Publicación</th><th>Acceso</th><th>Plano</th><th>Verificación</th><th>Nota</th></tr></thead><tbody>${D.fuentes.map(f => `<tr><td>${f.url ? `<a href="${esc(f.url)}" target="_blank" rel="noopener">${esc(f.nombre)}</a>` : esc(f.nombre)}<div class="body-s muted">${esc(f.institucion)}</div></td><td>${esc(f.fecha_publicacion || f.anio_referencia)}</td><td class="body-s" style="font-family:var(--mono)">${esc(f.via_acceso)}</td><td><span class="badge ${f.plano_evidencia === 'documentado' ? 'pri' : 'err'}">${esc(f.plano_evidencia)}</span></td><td class="body-s" title="${verTitulo(f.verificacion)}">${sello(f.verificacion)}</td><td class="body-s muted">${esc(f.nota_homologacion)}</td></tr>`).join('')}</tbody></table>`;
